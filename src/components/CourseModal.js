...
export default function CourseModal({ onClose, toast, geminiKey, institutions = [], initialData = null }) {
    ...
    const generateDescriptionWithAI = async () => {
        ...
        const prompt = `
            אתה מומחה לפדגוגיה חדשנית בחברת "חותם חיים מבית רובוטיקס".
                            ...
                            <div className="flex flex-wrap gap-2">
                                {availableFields.map(field => (
                                    <button 
                                        ...
                                        className={`px-3 py-1.5 rounded-full text-sm font-bold border-2 transition-colors ${fields.includes(field) ? 'border-purple-600 bg-purple-50 text-purple-700' : 'border-slate-200 text-slate-500 hover:border-slate-300'}`}>
                                        {field}
                                    </button>
                                ))}
                            </div>
