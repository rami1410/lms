...
export default function LessonEditor({ activeLesson, setActiveLesson, isEditMode, lessons, setLessons, saveChanges, duplicateActiveLesson, deleteActiveLesson, toggleComplete, userProgress, getLessonIcon }) {
    ...
    // פונקציית עזר חכמה המבצעת קרופ אגרסיבי במיוחד למניעת נראות של יוטיוב
        ...
        return (
            <div className="relative w-full rounded-3xl shadow-xl border overflow-hidden bg-black group" style={{ paddingTop: '56.25%' }}>
                <iframe 
                    ...
                    allowFullScreen 
                    /* החיתוך הוגדל משמעותית ל-130% גובה ו-112% רוחב כדי להעלים הכל לחלוטין */
                    style={{
                        width: '112%',
                        height: '130%',
                        top: '-15%',
                ...
                {/* חומות המגן עודכנו בהתאם לשטחי החיתוך החדשים */}
                {isYouTube && (
                    <>
                        <div className="absolute top-0 left-0 w-full h-16 bg-transparent z-10" title="וידאו מאובטח"></div>
                        <div className="absolute bottom-0 left-0 w-full h-16 bg-transparent z-10"></div>
                    </>
                )}
            </div>
        );
