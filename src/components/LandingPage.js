    ...
    useEffect(() => {
        ...
        function initPageLogic() {
            ...
            if (THREE && canvasContainerRef.current && !renderer) {
                ...
                // הצורות הגיאומטריות המעודכנות שלך
                const geometries = [
                ...
                geometries.forEach((geo, i) => {
                    const mat = new THREE.MeshPhongMaterial({ 
                        ...
                        transparent: true, 
                        opacity: 0,
                        wireframe: true // <--- הפקודה שהופכת את *כל* הצורות לרשת עדינה!
                    });
                ...
                const handleScroll = () => {
                    ...
                    objects.forEach((obj, i) => {
                        ...
                        if (rect.top < vh && rect.bottom > 0) {
                            obj.material.opacity = Math.min(1, 1 - Math.abs(centerOffset));
                            const scaleFactor = 1 + (1 - Math.abs(centerOffset)) * 1.5;
                            obj.scale.set(scaleFactor, scaleFactor, scaleFactor);
                            obj.position.z = (1 - Math.abs(centerOffset)) * 2;
                            obj.rotation.z = centerOffset * 0.5;
                        } else {
    ...
    return (
        <div className="antialiased bg-white text-[#1a1a1a] font-sans" dir="rtl">
            <style>{`
                ...
                .scroll-indicator { position: absolute; bottom: 30px; left: 50%; transform: translateX(-50%); text-align: center; color: #1a1a1a; opacity: 0.6; animation: bounce 2s infinite; }
            ...
            <header className="fixed top-0 w-full bg-white/90 backdrop-blur-md shadow-sm z-[100] no-print">
                <div className="max-w-7xl mx-auto px-4 flex justify-between items-center h-24">
                    ...
                    <div className="flex gap-4 items-center">
                        ...
                        {/* הוספתי חזרה את כפתור ההתחברות ताकि יהיה אפשר להיכנס למערכת! */}
