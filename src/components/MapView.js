...
export default function MapView({ courses, direction, mapBackground }) {
    ...
    const coursePositions = useMemo(() => {
        ...
        // הגדרת "אזורים" במפה לפי אחוזים (x, y) - התאמתי את אלו למפה החדשה
        const zones = [
            { x: 15, y: 15 }, // איים צפים
            ...
            return course.prerequisites.map(preName => {
                ...
                return (
                    <line 
                        ...
                        x2={`${pos.x}%`} y2={`${pos.y}%`} 
                        stroke="rgba(255,255,255,0.7)" strokeWidth="3" strokeDasharray="8,4"
                    />
                );
            });
        });
    ...
    return (
        <div className="relative w-full aspect-video rounded-[3rem] overflow-hidden shadow-2xl border-4 border-white group bg-slate-200">
            {/* הרקע - המפה הבסיסית - עמעמתי אותה פחות, כעת היא בהירה! */}
            <img src={mapBackground} className="absolute inset-0 w-full h-full object-cover transition-transform duration-[20s] group-hover:scale-110 opacity-100" alt="Learning Map" />
            
            {/* הסרתי את שכבת העמעום כליל כדי שתהיה בהירה ככל האפשר */}
            ...
            {courses.map(course => {
                ...
                return (
                    ...
                    >
                        {/* שיפרתי את נראות הישוב עם צללית ברורה */}
                        <div className="relative flex flex-col items-center">
                            <span className="text-3xl drop-shadow-[0_2px_10px_rgba(0,0,0,0.8)]">{icon}</span>
                            
                            {/* שם הקורס מופיע כעת בקופסה לבנה קבועה מתחת לישוב, גלוי תמיד! */}
                            <div className="bg-white/90 backdrop-blur-md px-3 py-1 rounded-full shadow-lg border border-white/50 mt-1 whitespace-nowrap">
            ...
            {/* מקרא צבעים */}
            <div className={`absolute bottom-6 ${direction === 'rtl' ? 'right-6' : 'left-6'} bg-white/80 backdrop-blur-md p-4 rounded-[2rem] shadow-xl border border-white/40 hidden md:block`}>
                <p className="text-[10px] font-black text-slate-400 mb-2 uppercase tracking-tighter">מקרא מקצועות</p>
